/*
 * Class BOUNDED [NATURAL_8]
 */

#include "eif_macros.h"


#ifdef __cplusplus
extern "C" {
#endif

static EIF_TYPE_INDEX gen_type0_921 [] = {459,920,424,0xFFFF};
static EIF_TYPE_INDEX gen_type1_921 [] = {920,424,0xFFFF};
static EIF_TYPE_INDEX gen_type2_921 [] = {0,0xFFFF};
static EIF_TYPE_INDEX gen_type3_921 [] = {0,0xFFFF};
static EIF_TYPE_INDEX gen_type4_921 [] = {920,424,0xFFFF};
static EIF_TYPE_INDEX gen_type5_921 [] = {920,424,0xFFFF};
static EIF_TYPE_INDEX gen_type6_921 [] = {0,0xFFFF};
static EIF_TYPE_INDEX gen_type7_921 [] = {920,424,0xFFFF};
static EIF_TYPE_INDEX gen_type8_921 [] = {920,424,0xFFFF};
static EIF_TYPE_INDEX gen_type9_921 [] = {913,0xFFF8,1,0xFFFF};
static EIF_TYPE_INDEX gen_type10_921 [] = {0xFFF8,1,0xFFFF};


static struct desc_info desc_921[] = {
	{(BODY_INDEX) 2871, (BODY_INDEX) -1, INVALID_DTYPE, NULL},
	{0, (BODY_INDEX)-1, 359, NULL},
	{1, (BODY_INDEX)-1, 459, gen_type0_921},
	{2, (BODY_INDEX)-1, 346, NULL},
	{3, (BODY_INDEX)-1, 346, NULL},
	{4, (BODY_INDEX)-1, 346, NULL},
	{5, (BODY_INDEX)-1, 346, NULL},
	{6, (BODY_INDEX)-1, 346, NULL},
	{7, (BODY_INDEX)-1, 346, NULL},
	{8, (BODY_INDEX)-1, 346, NULL},
	{9, (BODY_INDEX)-1, 346, NULL},
	{10, (BODY_INDEX)-1, 920, gen_type1_921},
	{11, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{12, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13, (BODY_INDEX)-1, 0, gen_type2_921},
	{14, (BODY_INDEX)-1, 0, gen_type3_921},
	{15, (BODY_INDEX)-1, 920, gen_type4_921},
	{16, (BODY_INDEX)-1, 920, gen_type5_921},
	{17, (BODY_INDEX)-1, 0, gen_type6_921},
	{18, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{19, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{20, (BODY_INDEX)-1, 42, NULL},
	{21, (BODY_INDEX)-1, 359, NULL},
	{22, (BODY_INDEX)-1, 359, NULL},
	{23, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{24, (BODY_INDEX)-1, 41, NULL},
	{25, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{26, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{27, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{28, (BODY_INDEX)-1, 920, gen_type7_921},
	{29, (BODY_INDEX)-1, 349, NULL},
	{30, (BODY_INDEX)-1, 920, gen_type8_921},
	{31, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{32, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{(BODY_INDEX)-1, (BODY_INDEX)-1, 346, NULL},
	{2840, (BODY_INDEX)-1, 346, NULL},
	{2800, (BODY_INDEX)-1, 346, NULL},
	{2801, 0, 346, NULL},
	{2802, (BODY_INDEX)-1, 346, NULL},
	{2803, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{2804, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{(BODY_INDEX)-1, (BODY_INDEX)-1, 913, gen_type9_921},
	{(BODY_INDEX)-1, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, gen_type10_921},
	{(BODY_INDEX)-1, (BODY_INDEX)-1, 400, NULL},
	{(BODY_INDEX)-1, (BODY_INDEX)-1, 346, NULL},
	{2870, (BODY_INDEX)-1, 346, NULL},
	{(BODY_INDEX)-1, (BODY_INDEX)-1, 400, NULL},
};

extern void Init921(void);
void Init921(void)
{
	IDSC(desc_921, 0, 920);
	IDSC(desc_921 + 1, 4, 920);
	IDSC(desc_921 + 34, 364, 920);
	IDSC(desc_921 + 43, 240, 920);
	IDSC(desc_921 + 45, 444, 920);
	IDSC(desc_921 + 46, 359, 920);
}


#ifdef __cplusplus
}
#endif
