/*
 * Class WSF_URI_TEMPLATE_MAPPING
 */

#include "eif_macros.h"


#ifdef __cplusplus
extern "C" {
#endif

static EIF_TYPE_INDEX gen_type0_380 [] = {459,379,0xFFFF};
static EIF_TYPE_INDEX gen_type1_380 [] = {0,0xFFFF};
static EIF_TYPE_INDEX gen_type2_380 [] = {0,0xFFFF};
static EIF_TYPE_INDEX gen_type3_380 [] = {0,0xFFFF};
static EIF_TYPE_INDEX gen_type4_380 [] = {365,0xFFFF};


static struct desc_info desc_380[] = {
	{(BODY_INDEX) 13222, (BODY_INDEX) -1, INVALID_DTYPE, NULL},
	{0, (BODY_INDEX)-1, 359, NULL},
	{1, (BODY_INDEX)-1, 459, gen_type0_380},
	{2, (BODY_INDEX)-1, 346, NULL},
	{3, (BODY_INDEX)-1, 346, NULL},
	{4, (BODY_INDEX)-1, 346, NULL},
	{5, (BODY_INDEX)-1, 346, NULL},
	{6, (BODY_INDEX)-1, 346, NULL},
	{7, (BODY_INDEX)-1, 346, NULL},
	{8, (BODY_INDEX)-1, 346, NULL},
	{9, (BODY_INDEX)-1, 346, NULL},
	{10, (BODY_INDEX)-1, 379, NULL},
	{11, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{12, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13, (BODY_INDEX)-1, 0, gen_type1_380},
	{14, (BODY_INDEX)-1, 0, gen_type2_380},
	{15, (BODY_INDEX)-1, 379, NULL},
	{16, (BODY_INDEX)-1, 379, NULL},
	{17, (BODY_INDEX)-1, 0, gen_type3_380},
	{18, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{19, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{20, (BODY_INDEX)-1, 42, NULL},
	{21, (BODY_INDEX)-1, 359, NULL},
	{22, (BODY_INDEX)-1, 359, NULL},
	{23, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{24, (BODY_INDEX)-1, 41, NULL},
	{25, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{26, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{27, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{28, (BODY_INDEX)-1, 379, NULL},
	{29, (BODY_INDEX)-1, 349, NULL},
	{30, (BODY_INDEX)-1, 379, NULL},
	{31, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{32, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13208, (BODY_INDEX)-1, 359, NULL},
	{13213, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13215, (BODY_INDEX)-1, 355, NULL},
	{13219, 8, 242, NULL},
	{13217, (BODY_INDEX)-1, 358, NULL},
	{13218, (BODY_INDEX)-1, 346, NULL},
	{13211, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13209, (BODY_INDEX)-1, 355, NULL},
	{13210, (BODY_INDEX)-1, 16, NULL},
	{13214, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13216, 0, 365, NULL},
	{13220, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13221, (BODY_INDEX)-1, (EIF_TYPE_INDEX)-1, NULL},
	{13212, (BODY_INDEX)-1, 365, gen_type4_380},
	{13219, 8, 242, NULL},
};

extern void Init380(void);
void Init380(void)
{
	IDSC(desc_380, 0, 379);
	IDSC(desc_380 + 1, 4, 379);
	IDSC(desc_380 + 34, 106, 379);
	IDSC(desc_380 + 35, 316, 379);
	IDSC(desc_380 + 42, 410, 379);
	IDSC(desc_380 + 43, 438, 379);
	IDSC(desc_380 + 48, 345, 379);
}


#ifdef __cplusplus
}
#endif
